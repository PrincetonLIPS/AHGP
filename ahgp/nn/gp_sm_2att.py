...
class GpSMDoubleAtt(nn.Module):

  def __init__(self, config):
    super(GpSMDoubleAtt, self).__init__()
    ...
    dim_list_mu = [self.att2_hidden_dim] + self.hidden_dim_mu + [self.num_mix]
    ...
    self.filter_mu = nn.ModuleList([
      Linear(dim_list_mu[tt], dim_list_mu[tt + 1])
      for tt in range(self.num_layer_mu+1)
    ])
  ...
  def forward(self, X_data_tr, X_data_val, F_data_tr, F_data_val, node_mask_tr, dim_mask, kernel_mask_val, diagonal_mask_val, N_data_val, device=torch.device('cpu'), eval_mode = False, X_data_test = None, F_data_test = None, kernel_mask_test=None, diagonal_mask_test=None, N_data_test=None):
    ...
    state_mu = dim_encoder_input.clone()
    ...
    for tt in range(self.num_layer_mu):
      state_mu = F.relu(self.filter_mu[tt](state_mu))
      state_mu = F.dropout(state_mu, self.dropout, training=self.training)
    ...
    mu = self.filter_mu[-1](state_mu)
    mu = torch.clamp(mu,min=-10.0,max=10.0)
    mu = torch.exp(mu)
    mu = mu * dim_mask.unsqueeze(-1)
    ...
    mu = mu.permute(0,2,1) # B X M X D
    weights = weights.permute(0,2,1) # B X M X D
    K_val = cal_kern_spec_mix_sep(X_data_val, X_data_val, mu, var, weights)
    ...
    if eval_mode:
      ...
      K_test = cal_kern_spec_mix_sep(X_data_test, X_data_test, mu, var, weights)
      nmll_test = -cal_marg_likelihood(K_test, f_test, self.epsilon, kernel_mask_test, diagonal_mask_test, N_data_test.float(), device)
      return mu, var, weights, nmll, nmll_test

    return mu, var, weights, nmll
    """
    output shape: B X 0
