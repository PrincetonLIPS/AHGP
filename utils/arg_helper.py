...
def parse_arguments():
  ...
  parser.add_argument('-v', '--val', help="Val model", action='store_true')
  parser.add_argument('-opt', '--opt', help="MLL Opt model", action='store_true')
...
def get_config(config_file, exp_dir=None):
  ...
  config.exp_name = '_'.join([
      config.model.name, config.dataset.name, str(config.train.lr), config.exp_name,
...
def get_bo_config(config_file, exp_dir=None):
  ...
  config.run_id = str(os.getpid())
  config.exp_name = '_'.join([
      config.model.name, config.dataset.name, config.exp_name,
      time.strftime('%Y-%b-%d-%H-%M-%S'), config.run_id
  ])
